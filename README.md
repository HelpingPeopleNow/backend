...
| GET | `/livez` | No | Liveness probe for the docker container healthcheck. Returns 200 while the process and Postgres are up; **does not** check the helper/LLM (so a helper outage doesn't kill the API). See `infra/AGENTS.md` and `infra/docs/FOLLOW_UP_SPOF.md` for the SPOF Phase framing. |
