...
- **OpenAPI spec (spec-first)** — `openapi/openapi.yaml` is the hand-authored OpenAPI 3.0.3 contract, embedded via `go:embed` (`openapi/openapi.go`) and served at `/api/v1/openapi.yaml` (session-auth'd) with an interactive Redoc UI at `/api/v1/docs` (admin-only). **When you add/change a route in `buildMux`, update `openapi.yaml` in the same commit** — `openapi_handler_test.go` parses the spec and asserts the core routes exist, so a broken or missing spec entry fails CI. Paths are mounted in `buildMux` via `handler.OpenAPISpecHandler()` / `handler.OpenAPIDocsHandler()` (`internal/adapters/handler/openapi_handler.go`).
...
- **Profession normalizer** — `core.ProfessionAliases` (`internal/core/professions.go`) is the single source of truth for EN/ES profession normalization. Maps lowercase aliases (e.g. `electricista`, `fontanero`, `pintor`) to canonical English names (`Electrician`, `Plumber`, `Painter`). `NormalizeProfession()` does case-insensitive lookup; unknown values pass through unchanged. `NormalizeProfessionForEmbedding()` is a thin wrapper that delegates to it. The map must stay in sync with `helper/scripts/backfill_embeddings.py` so batch backfill and runtime queries canonicalize to identical strings.
...
**`health_status` gauge is kept LIVE by a background poller** (`startHealthPoller` in `main.go`, every 10s, on `rootCtx`/`rootWG`). Historically the gauge was only updated inside the `/health` handler, so a transient dependency blip left a stale `0` in Prometheus until the next `/health` call — producing false "down" in Grafana. The poller probes postgres + helper health and refreshes `health_status{component=postgres|grpc_helper}` continuously, so the gauge reflects real state within ~10s of any change (verified: stops showing false-down after a helper restart with no `/health` traffic). The `/health` handler still calls `SetHealthStatus` for an immediate refresh on a manual probe. (P3-2: the `HostResources` dashboard health panel + any future `health_status==0` alert now fire truthfully.)
...
- **pgvector extension** auto-installed by `database.Connect()` on startup. HNSW index `idx_worker_embeddings_hnsw` (m=16, ef_construction=64) auto-created with cosine distance. `hnsw.ef_search` is set to 64 (default was 40; tune up when corpus exceeds ~1k vectors).
- **`worker_embeddings` table** is `core.WorkerEmbedding` (composite PK `user_id`+`field_name`, `embedding vector(768)`, `model`, `text_hash` SHA-256 hex, `timestamptz updated_at`). One row per embeddable field per worker (up to 7 fields).
- **`BuildFieldTexts()`** (`internal/core/worker_embeddings.go`) maps a `WorkerProfile` to a `map[string]string` of field name → embedding text. Fields: `profession` (normalized via `NormalizeProfessionForEmbedding`), `profession_raw` (original text, at lower weight — ensures unrecognized professions still rank in vector space), `bio`, `certifications` (joined from JSON array), `city`, `languages` (joined from JSON array), `business_name`. Empty fields are excluded entirely (no row produced).
- **`FieldWeights`** (`internal/core/worker_embeddings.go`) — `map[string]float64` mapping each embeddable field to its contribution weight in the hybrid search score. `profession=1.0`, `bio=0.8`, `certifications=0.7`, `city=0.4`, `profession_raw=0.3`, `languages=0.3`, `business_name=0.3`. Hardcoded for V1.
...
- **Two-pass search with parallel Pass-1 + Embed** — `SearchService.Search` runs Pass-1 (LLM filter extraction) and Embed (raw message → vector) concurrently via `errgroup`. The embed result is cached by `sha256(message)` so identical resubmits skip the embedding call. If embed fails, `filters.EmbedFailed` is set and FindWorkers returns `branch='ilike_embed_failed'` (F6).
- **Branch selection** — `ProfileRepository.FindWorkers` returns `FindResult{Branch, Workers, TopScore}` where `Branch` is one of: `"vector"`, `"ilike"`, `"ilike_disabled_via_env"`, `"ilike_low_top_score"`, `"ilike_embed_failed"`, `"ilike_fallback"`. Selection is post-fact (in the repo), not pre-fact (in the service), so the slog branch reflects what actually ran.
- **Caches** — Two-tier: (1) Pre-key cache: `sha256(message+city)` → cached Pass-1 response; identical repeat queries skip both LLM calls entirely. (2) Embed-result cache: `sha256(message)` → cached vector; skips embed call. Both bounded at 200 entries with lazy oldest-eviction.
- **Metrics** — `vector_search_total{branch=...}` counter and `vector_score` histogram (wired in `internal/adapters/handler/metrics_handler.go` and incremented from `ChatHandler`). Also: `embed_failures_total` counter, `search_cache_size` gauge.
- **Re-backfill** on schema change or after first enable: `docker exec helpingpeoplenow-helper env DB_HOST=helpingpeoplenow-postgres DB_USER=postgres DB_PASSWORD=postgres DB_NAME=helpingpeoplenow HELPER_GRPC_ADDR=localhost:50051 python3 /app/scripts/backfill_embeddings.py` (idempotent — skips rows whose `text_hash` matches existing). The byte-parity gate (`helper/scripts/test_byte_parity_gate.sh`) ensures Go's `BuildFieldTexts` and Python's `backfill_embeddings.py` produce identical field texts and SHA-256 hashes.
- **Test coverage** — `search_service_test.go` (30+ test functions covering cache behavior, branch selection, filter extraction, parallel execution, embed failure). `tests/integration/repository_test.go` (PG-backed tests for ILIKE fallback, vector branch retention, low top-score fallback, embed failure branch). `internal/core/professions_test.go` (parity test covering all `ProfessionAliases` keys).
