...
func TestSearchEmptyPrompt(t *testing.T) {
	...
	result, err := svc.Search(t.Context(), "user-1", "plumber", nil, nil, "", "", nil, nil)
...
func TestSearchConversationalNoUserID(t *testing.T) {
	...
	result, err := svc.Search(t.Context(), "", "hi there", nil, nil, "", "", nil, nil)
	require.NoError(t, err)
...
func TestSearchCacheHit(t *testing.T) {
	...
	// First call
	result1, err := svc.Search(t.Context(), "user-1", "plumber", nil, nil, "", "", nil, nil)
	require.NoError(t, err)

	// Second call with same query — should hit cache
	result2, err := svc.Search(t.Context(), "user-1", "plumber", nil, nil, "", "", nil, nil)
	require.NoError(t, err)
