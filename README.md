...
|--------|------|------|-------------|
| GET | `/health` | No | Health check (composite: PG ping + helper gRPC `health`). Returns 200 when both deps are `ok`; 503 when either is `down`. Response: `{"status":"ok"\|"degraded", "postgres":"ok"\|"down", "grpc_helper":"ok"\|"down", "details":{<key>:<err>}}` — `details` is a **map[string]string** keyed by `postgres_err` / `grpc_helper_err`. `Content-Type: application/json`. **NOT used** for the docker container healthcheck — the `/livez` endpoint below is the correct container-level target so a helper outage does NOT cascade to a full API outage. |
| GET | `/livez` | No | Liveness probe for the docker container healthcheck. Returns 200 while the process and Postgres are up; **does not** check the helper/LLM (so a helper outage doesn't kill the API). See `infra/AGENTS.md` and `infra/docs/FOLLOW_UP_SPOF_Backup_Replicas.md` for the SPOF Phase framing. |
| GET | `/readyz` | No | Traefik load-balancer health-check target. Backed by a singleton `*atomic.Bool` flipped to `true` once `main.buildMux` returns (after system prompts are seeded + mux wired) and flipped back to `false` by `handler.MarkUnready()` from `main.runShutdownSequence()` on SIGTERM. Returns 200 (`{"ready":true}`) when the LB should route traffic to this replica; 503 (`{"ready":false}`) when it should be drained. Wired in `infra/traefik-dev-dynamic.yaml` (dev) and via `traefik.http.services.backend.loadbalancer.healthcheck.*` Docker labels on the backend service (10s interval, 3s timeout). |
