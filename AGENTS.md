...
- **SystemPromptHandler PUT is admin-protected, GET is auth-only** — `main.go:73-74` wraps only the `/api/v1/system-prompts/` (trailing slash) routes in `d.Admin.Wrap(...)` (used by PUT). The bare `/api/v1/system-prompts` route (used by GET) is wrapped only by `d.Auth.Wrap(...)`, so any authenticated user can read prompts + LLM provider.
